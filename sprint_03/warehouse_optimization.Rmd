...
```{r include=FALSE}
# test <- vk_zg_class %>% group_by(lieferschein) %>% arrange(materialnummer) %>% dplyr::summarize(mat_comb=paste0(unique(materialnummer), collapse = ";"))
 
# write.csv(test %>% select(lieferschein, mat_comb),"./comb.csv", quote = FALSE, row.names = FALSE)
```
...
# Vergleich einzelne Zugriffe vs. ABC Zugriffe

Wenn wir stark modellarisieren und annehmen, dass alle Materialien in einem Lieferschein, die in einer gemeinsamen ABC-Zone liegen einen einzelnen Zugriff ausmachen.
Und es mit Zugriffen vergleichen, bei dem jedes Material in einem Lieferschein als einzelner Zugriff gewertet wird, erhalten wir folgende Informationen:

```{r, include=FALSE}
unique_zugriffe_ls <- verkaeufe %>% group_by(lieferschein) %>% summarize(unique_mat=n_distinct(materialnummer))

unique_zugriffe_ls
```

```{r, echo=FALSE}
sum_zugriffe <- sum(unique_zugriffe_ls$unique_mat)

print(paste("Einzelne Zugriffe =", sum_zugriffe))
```

```{r include=FALSE}
zugriffe <- class_ls_count %>% mutate(length_klasse=lengths(str_split(komb_klasse_zugriffe, pattern=",")))

zugriffe <- zugriffe %>% mutate(zugriffe_class=anzahl_ls_mit_komb*length_klasse)

zugriffe
```

```{r echo=FALSE}
sum_zugriffe_zoning <- sum(zugriffe$zugriffe_class)

print(paste("ABC-Zonung Zugriffe =", sum_zugriffe_zoning))
```

```{r echo=FALSE}
print(paste("ABC-Zonung/Einzelne Zugriffe =", sum_zugriffe_zoning/sum_zugriffe)) 
print(paste("Bei ABC-Zonung nur", round((sum_zugriffe_zoning/sum_zugriffe)*100), "Prozent der Zugriffe nötig")) 
print(paste("Effizenzsteigerung von", round((1-sum_zugriffe_zoning/sum_zugriffe)*100), "Prozent")) 
```
