...
weird <- verkaeufe %>%  group_by(lieferschein) %>%  arrange(materialnummer) %>% summarize(length_comb = n_distinct(materialnummer), comb=paste0(unique(materialnummer), collapse = ","))  %>% arrange(desc(length_comb))

weird

#weird %>% filter(lieferschein == '107163630')

weird %>% group_by(length_comb) %>% summarize(count=n())
