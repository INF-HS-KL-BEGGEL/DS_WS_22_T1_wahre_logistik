...
con_postgres <- DBI::dbConnect(odbc::odbc(),
                      Driver   = Sys.getenv("driver"),
                      Server   = Sys.getenv("host"),
...
```{r}
#Lade Cities und Kunden
cities <- dbGetQuery(conn=con_postgres, statement = "select * from cities_de")
kunden <- dbGetQuery(conn=con_postgres, statement = "select * from kundenstammdaten")
```

```{r}
#Kreuze Cities und Kunden
cxk <- crossing(kunden, cities)
```

```{r}
#Erzeuge Spalte distance mit euklidischer Distanz zwischen den beiden long, lat werten aus Cities und Kunden
cxk <- cxk %>% mutate(distance=sqrt((laengengrad-lng)^2 + (breitengrad-lat)^2))
```

```{r}
#Wähle nur die kleinste Distanz aus
cxk <- cxk %>%  group_by(kndnr) %>%  slice_min(distance, with_ties = FALSE) %>% arrange(desc(distance))
#Selectiere nur kdnr und cid für das Update in der DB 
scxk <- cxk %>% select(kndnr, cid)
```

```{r}
#Weise den Kundendatensätze die entsprechende Cid aus cities zu
for(i in 1:nrow(scxk)) {
  res <- dbSendQuery(con_postgres, paste("UPDATE kundenstammdaten SET city_id=",pull(scxk[i,2])," WHERE kndnr='",pull(scxk[i,1]),"';", sep=""))
  dbClearResult(res)
  #print(paste(i, "done"))
}
```
...
```{r}
#Funktion zum Berechnen der kürzesten Strecke auf der Erde (Elliptische Geometrie)
orthodrome_earth_meters <- function(lat1, long1, lat2, long2) {
...
# #Test: Berlin-Tokio - Ergebnis sollte ca.8918000m
