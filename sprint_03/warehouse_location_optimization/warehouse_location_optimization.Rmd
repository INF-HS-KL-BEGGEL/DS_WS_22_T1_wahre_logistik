...
Das Raster umfasst `r raster_points %>% nrow()` Punkte, die sowohl in x- als auch y-Richtung jeweils einen Abstand von ca. 86,3 km haben.
...
tmp3 <-
  raster_points %>% 
  filter(long == raster_points[1,]$long) %>% 
  arrange(lat)
```

```{r include=FALSE}
for(i in 1:10){
  dist = orthodrome_meters(lat1 = tmp3[i,]$lat,
                            long1 = tmp3[i,]$long,
                            lat2 = tmp3[i+1,]$lat,
                            long2 = tmp3[i+1,]$long)
...
leaflet(width = width_for_leaflet_maps, height = height_for_leaflet_maps) %>%  
  ...
  addCircleMarkers(lat = potential_warehouse_locations$proj_lat,
                   ...
                   radius = 6)
```
Beim Mapping sind zum Teil auch verschiedene Rasterpunkte auf denselben Straßenknoten abgebildet worden. Deshalb wurden aus den `r raster_points %>% nrow()` Rasterpunkten `r potential_warehouse_locations %>% nrow()` Standortkandidaten in Deutschland.
...
kunden_ohne_stammdaten <- dbGetQuery(conn = con_to_normal_db, statement = query)
...
```{r include=FALSE}
kunden_ohne_stammdaten_enhanced <-
  umsatz_je_kunde %>%
  mutate(anteil_am_gesamtumsatz = umsatz_durch_kunde/sum(umsatz_durch_kunde)) %>% 
  arrange(desc(anteil_am_gesamtumsatz)) %>%
  mutate(kumulierter_anteil = cumsum(anteil_am_gesamtumsatz),
         rang = row_number()) %>% 
  inner_join(kunden_ohne_stammdaten, by = "kndnr") %>% 
  arrange(rang) 
```
...
Die betreffenden Kunden gehören nicht zu unseren umsatzstarken Kunden. Selbst zusammen machen sie gerade einmal `r round(sum(kunden_ohne_stammdaten_enhanced$anteil_am_gesamtumsatz) * 100, digits = 2)` % am Gesamtumsatz aus.
...
1. alle Lager darstellen und die, die weggefallen sind einfärben
...
start_coord_lats = tmp$lat
start_coord_longs = tmp$long
...
## Nicht routbare Kunden
```{r include=FALSE}
con_to_normal_db <- get_con_to_normal_db()
query_string <- "
                SELECT DISTINCT(end_vid) AS proj_vertex_id
                FROM routing_agg_results
                ;
                "
routable_customer_cluster_vertex_ids <- dbGetQuery(conn = con_to_normal_db, statement = query_string)
```


```{r include=FALSE}
non_routable_customer_cluster <-
  customer_cluster_proj %>% 
  filter(!proj_vertex_id %in% as.integer(routable_customer_cluster_vertex_ids$proj_vertex_id))
```
...
con_to_normal_db <- get_con_to_normal_db()
query <- "
          SELECT kndnr_tagless as kndnr, laengengrad as long, breitengrad as lat
        	FROM kundenstammdaten
        	;
        "


kundenstammdaten <- dbGetQuery(conn = con_to_normal_db, statement = query)
dbDisconnect(con_to_normal_db)
```


```{r include=FALSE}
non_routable_customers <-
  non_routable_customer_cluster %>% 
  inner_join(kundenstammdaten, by = c("original_lat" = "lat", "original_long" = "long"))
```
...
umsatz_je_kunde_enhanced <- 
  umsatz_je_kunde %>% 
  mutate(anteil_am_gesamtumsatz = umsatz_durch_kunde/sum(umsatz_durch_kunde)) %>% 
  arrange(desc(anteil_am_gesamtumsatz)) %>% 
  mutate(anteil_kumuliert = cumsum(anteil_am_gesamtumsatz),
         rang = row_number())
```


```{r include=FALSE}
non_routable_customers_enhanced <- 
  non_routable_customers %>% 
  select(-proj_vertex_id, -proj_lat, -proj_long, -proj_the_geom, -original_lat, -original_long) %>% 
  inner_join(umsatz_je_kunde_enhanced, by = "kndnr") %>% 
  arrange(rang)
```
...
Die nicht routbaren Kunden sind hinsichtlich des Umsatzes nicht zu vernachlässigen. Zusammen machen sie `r round(sum(non_routable_customers_enhanced$anteil_am_gesamtumsatz) * 100, digits = 2)` % vom Gesamtumsatz aus





## Nicht routbare Lagerkandidaten
Todo
...
Todo: vielleicht die Ränge dauerhaft anzeigen (z. B. im Circle)
...
```
Todo: Vielleicht die Lager noch dazu (und die Kundencluster dann einfach purple oder so)
