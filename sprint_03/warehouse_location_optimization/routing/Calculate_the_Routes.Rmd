          ...
          -- Hole zu allen start_coords den jeweils nächstgelegenen Node
           WITH start_vids AS (
            ...
            SELECT start_vid,
                  ...
                  proj_end_long,
                  proj_end_lat,
                  sum(ST_LengthSpheroid(the_geom_edge, 'SPHEROID[\"GRS_1980\",6378137,298.257222101]')) as length_of_route_in_meters
            FROM pgr_bdDijkstra(
            	  'SELECT id, source, target, cost, reverse_cost
            	  FROM de_edges',
            	  ARRAY(SELECT id from start_vids), 
...
```{r}
tmp_string <- 
  get_bd_dijkstra_many_to_many_aggregated_results_query_string(
    start_coord_longs = start_coord_longs,
    start_coord_lats = start_coord_lats,
    destination_coord_longs = destination_coord_longs,
    destination_coord_lats = destination_coord_lats
