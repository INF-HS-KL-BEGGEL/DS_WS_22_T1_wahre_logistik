...
```{r}
potential_warehouse_locations[3,]
```
...
```{r}
x <- get_con_to_routing_db()
y <- dbGetQuery(conn = x, statement = "Select * from potential_warehouse_locations")
dbDisconnect(x)
```
...
```{r}
# tmp <- dbGetQuery(conn = con_to_routing_db, statement = "Select * from potential_warehouse_locations")
```


```{r}
# con_to_normal_db = get_con_to_normal_db()
# kunden <- dbGetQuery(conn = con_to_normal_db, statement = "select * from kundenstammdaten")
# dbDisconnect(con_to_normal_db)
```
