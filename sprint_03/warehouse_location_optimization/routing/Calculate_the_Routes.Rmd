---
title: "Routen berechnen, Ergebnisse aggregieren und abspeichern"
...
```{r include=FALSE}
# Verbindung zu unserer Datenbank für das Routing
...
stop("Willst du das wirklich ausführen? Die Berechnung dauert ca. 10 h. Abgesehen davon, werden Daten in die Datenbank geschrieben.") # Todo: Nach Ausführung wieder entkommentieren
...
# for(i in 1:2) {# Zum Testen # Todo: noch raus
  
  print(glue("Anfang for-Schleife, i = {i}"))
  ...
  dbAppendTable(conn = con_to_normal_db, name = "routing_agg_results", value = compact_results)
  dbDisconnect(con_to_normal_db)
  
  print(glue("Ende for-Schleife, i = {i}"))
}
...
## Angepasster Code wegen Timeouts
Für manche potenzielle Lagerstandorte wird obige Query nicht erfolgreich durchgeführt (vermutlich Timeouts). Deshalb hier nochmal der Code in einer angepassten Variante, bei der zwei kleinere statt der einen großen Query zum Berechnen der Routen abgesetzt werden.
```{r include=FALSE}
# Verbindung zu unserer Datenbank für das Routing
con_to_routing_db <- DBI::dbConnect(odbc::odbc(),
                      Driver   = Sys.getenv("driver"),
...
# Iteriere über alle potenziellen Lagerstandorte
for(i in 1:nrow(potential_warehouse_locations)) {
# for(i in c(10, 11, 18, 20, 21, 22, 26, 30, 33, 41, 45, 46, 63, 70)) {# Zum Testen # Todo: noch raus
  ...
  from_warehouses_query <- glue("{from_warehouses_query};")
  ...
  to_warehouses_query <- glue("{to_warehouses_query};")
  
  
  from_warehouses_results <- dbGetQuery(conn = con_to_routing_db, statement = from_warehouses_query)
  to_warehouses_results <- dbGetQuery(conn = con_to_routing_db, statement = to_warehouses_query)
  results <- rbind(from_warehouses_results, to_warehouses_results)
