...
abc_analyse <- function(.data, column, thresholds){
  ...
  # class_mappings <- c(expr(!!ensym(column) <= thresholds[1] ~ classes[1]))
  # for(i in 2:length(thresholds)-1){
  #   class_mappings <- c(class_mappings, expr(!!ensym(column) > thresholds[i-1] & !!ensym(column) <= thresholds[i]  ~ classes[i]))
  # }
  # class_mappings <- c(class_mappings, TRUE ~ classes[length(classes)])
...
abc_analyse_nach_zugriffen <- verkaeufe %>%
  group_by(materialnummer) %>%
  summarise(anzahl_zugriffe = n()) %>%
  abc_analyse("anzahl_zugriffe", thresholds = c(0.8, 0.9, 0.95))
```
