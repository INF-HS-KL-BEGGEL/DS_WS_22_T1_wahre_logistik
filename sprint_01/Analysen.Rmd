...
```{r}
print(Sys.getenv("driver"))
print(Sys.getenv("host"))
print(Sys.getenv("dbname"))
...
map <- leaflet(data = ks[1:10,]) %>% addTiles() %>%  addMarkers(~laengengrad, ~breitengrad, popup = ~as.character(kndnr), label = ~as.character(kndnr))
