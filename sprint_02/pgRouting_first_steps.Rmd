...
```{r include=FALSE}
coord <- list(lat=49.260058, lon=7.359225)

query <- glue(
          "select *, ST_X(the_geom) as lon, ST_Y(the_geom) as lat
          from de_edges_vertices_pgr
          order by the_geom <-> ST_SetSRID(ST_MakePoint({coord$lon},{coord$lat}), 4326)
          limit 1;"
          )
...
```{r echo=FALSE}
lat_values <- c(nearest_node$lat, coord$lat)
lon_values <- c(nearest_node$lon, coord$lon)
my_map %>% 
  addCircleMarkers(lat = lat_values, lng=lon_values) %>%
  addPolylines(lat = lat_values, lon_values, color="red")
```
