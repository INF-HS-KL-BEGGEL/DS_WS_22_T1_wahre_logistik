...
```{r include=FALSE}
umsatz_je_klasse <- abc_analyse_kunde_nach_umsatz %>% 
  group_by(klasse) %>% 
...
abc_analyse_kunde_nach_umsatz %>% 
  ...
  scale_x_continuous(breaks = seq(from=0, to=100, by=20)) +
  scale_y_continuous(breaks = seq(from=0, to=100, by=20)) +
  geom_label(data=umsatz_je_klasse, mapping = aes(x=50, y=15, label=glue("{umsatz_in_klasse_formatiert} €")), hjust="middle", vjust="middle", color="black", fill="white") + 
  coord_fixed(ratio=1) +
  labs(
