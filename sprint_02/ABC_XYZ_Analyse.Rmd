...
VariationskoeffizientNachWochen <- inner_join(x=VariationskoeffizientNachWochen,y=nachMaterialnummernProWoche,by="materialnummer")
...
#month
#ergänzen des gesamten Abgangs und der menge an Bestellungen
VariationskoeffizientNachMonaten <- inner_join(x=VariationskoeffizientNachMonaten,y=nachMaterialnummernProMonat,by="materialnummer")
...
subsetOfVariationskoeffizient <-select(VariationskoeffizientNachMonaten,Variationskoeffizient,anteil_artikel,klasse,lieferscheine.x)
subsetOfVariationskoeffizient <- subsetOfVariationskoeffizient[subsetOfVariationskoeffizient$lieferscheine.x == 1, ]
subsetOfVariationskoeffizient %>% 
  ggplot() +
  geom_line(mapping = aes(x=(anteil_artikel-minimumAnteil)*100, y=Variationskoeffizient, color=klasse),size=1) +
  geom_hline(yintercept = 0, size=0.8) + 
...
```{r include=FALSE}
#merge abc and xyz
matrixAnalyse <- inner_join(x=abc_analyse_material_nach_umsatz,y=VariationskoeffizientNachWochen,by="materialnummer")
matrixAnalyse

matrixAnalyse <- matrixAnalyse %>% select(klasse.x,klasse.y,materialnummer,umsatz)
matrixAnalyse <- matrixAnalyse %>% rename(abc_analyse=klasse.x, xyz_analyse=klasse.y)
matrixAnalyse <- matrixAnalyse %>% group_by(abc_analyse,xyz_analyse) %>% summarize(materialien=n(), umsatz=sum(umsatz))

#This is hardcoded! Maybe make columns and row titles of the matrix later dynamic.
#place the values in a matrix
castedMatrixAnalyse <- matrix(glue('{as.character(matrixAnalyse$materialien)} Artikel
{as.character(matrixAnalyse$umsatz)} €'), nrow = 4, dimnames = list(c("W","X","Y","Z"), c("A","B","C","D","E")))
```
